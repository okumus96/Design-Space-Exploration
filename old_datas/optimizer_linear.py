...
class AssignmentOptimizerNew:
    def __init__(self):
        print("Initialized AssignmentOptimizerNew with linear ECU-ECU costs and linear sensor/actuator costs")
        pass
    ...
    def calculate_cable_expressions(self, model, x, y, z, scs, ecus, sensors, actuators, cable_types, comm_matrix):
        """
        Calculate cable cost, distance, and latency as Gurobi linear expressions.
        Linearizes ECU-ECU quadratic terms y[j1]*y[j2] using binary z[j1,j2].
        Returns (cable_cost_expr, cable_distance_expr, latency_expr).
        """
        # Precompute lookups
        cost_map = {name: ct.cost_per_meter for name, ct in cable_types.items()}
        ...
        for j1 in range(len(ecus)):
            for j2 in range(j1 + 1, len(ecus)):
                ...
                ecu_ecu_cost_terms.append((j1, j2, cost))
                ecu_ecu_distance_terms.append((j1, j2, dist))
                ecu_ecu_latency_terms.append((j1, j2, lat))

        # base expressions from sensor/actuator cables
        cable_cost_expr = gp.quicksum(var * c for var, c in cable_cost_terms) if cable_cost_terms else 0
        cable_distance_expr = gp.quicksum(var * d for var, d in cable_distance_terms) if cable_distance_terms else 0
        latency_expr = gp.quicksum(var * l for var, l in latency_terms) if latency_terms else 0

        # Add ECU-ECU contributions using pre-created z variables (linearized y*y)
        if ecu_ecu_cost_terms:
            cable_cost_expr += gp.quicksum(z[j1, j2] * cost for (j1, j2, cost) in ecu_ecu_cost_terms)
            cable_distance_expr += gp.quicksum(z[j1, j2] * dist for (j1, j2, dist) in ecu_ecu_distance_terms)
            latency_expr += gp.quicksum(z[j1, j2] * lat for (j1, j2, lat) in ecu_ecu_latency_terms)
    ...
    def create_base_model(self, scs, ecus, sensors, actuators, comm_matrix, cable_types, verbose=False, time_limit=None, mip_gap=None):
        ...
        # z[j1, j2] = 1 if both ECUs j1 and j2 are used (linearization of y[j1]*y[j2])
        z = {}
        for j1 in range(n_ecu):
            for j2 in range(j1 + 1, n_ecu):
                # only create z for pairs with meaningful distance
                dist = self._get_distance(ecus[j1].location, ecus[j2].location)
                if dist <= 0:
                    continue
                z[j1, j2] = model.addVar(vtype=GRB.BINARY, name=f"z_{ecus[j1].id}_{ecus[j2].id}")
                model.addConstr(z[j1, j2] <= y[j1], name=f"z_leq_y1_{j1}_{j2}")
                model.addConstr(z[j1, j2] <= y[j2], name=f"z_leq_y2_{j1}_{j2}")
                model.addConstr(z[j1, j2] >= y[j1] + y[j2] - 1, name=f"z_geq_sum_{j1}_{j2}")
    ...
    def optimize(self, scs, ecus, sensors, actuators, cable_types, comm_matrix, enable_latency=True):
        ...
        cable_cost_expr, cable_distance_expr, latency_expr = self.calculate_cable_expressions(
            model, x, y, z, scs, ecus, sensors, actuators, cable_types, comm_matrix
        )
